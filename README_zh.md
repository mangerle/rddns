...
| **Linux ARMv5TE** (`armv5te-unknown-linux-musleabi`) | 早期 Marvell 芯片老 NAS（如旧群晖 DS211j）、工业网关 | 纯静态 musl 编译 |
| **Linux 32位 (x86)** (`i686-unknown-linux-musl`) | 遗留 32 位 Linux 工控机、32位 Alpine 容器 | 纯静态 musl 编译 |
...
| **FreeBSD x86_64** (`x86_64-unknown-freebsd`) | 64 位 pfSense、OPNsense 专业防火墙与 FreeBSD | 原生 FreeBSD 64位二进制 |
| **FreeBSD ARM64** (`aarch64-unknown-freebsd`) | 树莓派 4/5 或 ARM64 云主机运行的 FreeBSD/OPNsense | 原生 FreeBSD ARM64 二进制 |
| **FreeBSD 32位** (`i686-unknown-freebsd`) | 老旧 32 位 x86 工控机防火墙 | 原生 FreeBSD 32位二进制 |
| **Windows 64位 (x86_64)** (`x86_64-pc-windows-msvc`) | Windows 10 / 11 / Server (Intel / AMD) | 原生绿色单文件，解压即用 |
| **Windows 64位 (ARM64)** (`aarch64-pc-windows-msvc`) | 骁龙 X Elite / Copilot+ PC、Surface ARM、M系列虚拟机 | 原生 ARM64 绿色单文件，解压即用 |
